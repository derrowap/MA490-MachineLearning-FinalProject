...
hiddenSize = 20
hiddenArr = np.zeros((hiddenSize, 3))
for layers in range(1, hiddenSize+1):
    #generate data
    batchSize = 1000
    ...
    regressor = skflow.TensorFlowDNNRegressor(hidden_units=[layers], steps=10000, learning_rate=0.1)
    ...
    hiddenArr[layers-1][0] = layers
    hiddenArr[layers-1][1] = score
    hiddenArr[layers-1][2] = i

for i in range(hiddenSize):
    print('layer %d, MSE %f, failed at %d' % (hiddenArr[i][0], hiddenArr[i][1], hiddenArr[i][2]))
